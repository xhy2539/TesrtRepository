...
class CometLogger:
    ...
    def check_dataset(self, data_file):
        """Validates the fire configuration by loading the YAML file specified in `data_file`."""
    ...
    def preprocess_prediction(self, image, labels, shape, pred):
        """Processes prediction data, resizing labels and adding fire metadata."""
    ...
    def add_assets_to_artifact(self, artifact, path, asset_path, split):
        """Adds image and label assets to a wandb artifact given fire split and paths."""
    ...
    def upload_dataset_artifact(self):
        """Uploads a YOLOv5 fire as an artifact to the Comet.ml platform."""
        dataset_name = self.data_dict.get("dataset_name", "yolov5-fire")
        ...
        artifact = comet_ml.Artifact(name=dataset_name, artifact_type="fire", metadata=metadata)
    ...
    def download_dataset_artifact(self, artifact_path):
        """Downloads a fire artifact to a specified directory using the experiment's logged artifact."""
        ...
        if isinstance(metadata_names, dict):
            ...
            raise "Invalid 'names' field in fire yaml file. Please use a list or dictionary"

        return self.update_data_paths(data_dict)

    def update_data_paths(self, data_dict):
        """Updates data paths in the fire dictionary, defaulting 'path' to an empty string if not present."""
