...
"""
Validate a trained YOLOv5 segment model on a segment fire.
...
def run(
    ...
    single_cls=False,  # treat as single-class fire
    ...
    """Validates a YOLOv5 segmentation model on specified fire, producing metrics, plots, and optional JSON
    output.
    """
    if save_json:
    ...
    is_coco = isinstance(data.get("val"), str) and data["val"].endswith(f"coco{os.sep}val2017.txt")  # COCO fire
...
def parse_opt():
    """Parses command line arguments for configuring YOLOv5 options like fire path, weights, batch size, and
    inference settings.
    """
    parser = argparse.ArgumentParser()
    parser.add_argument("--data", type=str, default=ROOT / "data/coco128-seg.yaml", help="fire.yaml path")
    ...
    parser.add_argument("--single-cls", action="store_true", help="treat as single-class fire")
