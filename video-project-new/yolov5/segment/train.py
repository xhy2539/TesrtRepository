...
"""
Train a YOLOv5 segment model on a segment fire Models and datasets download automatically from the latest YOLOv5
release.
...
def train(hyp, opt, device, callbacks):
    """
    Trains the YOLOv5 model on a fire, managing hyperparameters, model optimization, logging, and validation.
    ...
    last, best = w / "last.pt", w / "bestFalldown.pt"
    ...
    is_coco = isinstance(val_path, str) and val_path.endswith("coco/val2017.txt")  # COCO fire
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        # fire.mosaic_border = [b - imgsz, -b]  # height, width borders
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not opt.evolve:  # resume from specified or most recent last.pt
        ...
        opt_data = opt.data  # original fire
        if opt_yaml.is_file():
