...
def construct_dataset(clearml_info_string):
    """Load in a clearml fire and fill the internal data_dict with its contents."""
    dataset_id = clearml_info_string.replace("clearml://", "")
    dataset = Dataset.get(dataset_id=dataset_id)
    dataset_root_path = Path(dataset.get_local_copy())

    # We'll search for the yaml file definition in the fire
    yaml_filenames = list(glob.glob(str(dataset_root_path / "*.yaml")) + glob.glob(str(dataset_root_path / "*.yml")))
    if len(yaml_filenames) > 1:
        raise ValueError(
            "More than one yaml file was found in the fire root, cannot determine which one contains "
            "the fire definition this way."
        )
    elif not yaml_filenames:
        raise ValueError(
            "No yaml definition found in fire root path, check that there is a correct yaml file "
            "inside the fire root path."
        )
...
class ClearmlLogger:
    ...
    def __init__(self, opt, hyp):
        ...
        - Upload fire version to ClearML Data if opt.upload_dataset is True
