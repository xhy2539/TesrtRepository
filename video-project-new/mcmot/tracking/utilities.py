...
def apply_homography_xyxy(xyxy, H):
    ...
    for idx, (x1, y1, x2, y2) in enumerate(xyxy):
        ...
        xyxy_[idx] = [round(x1[0], 2), round(y1[0], 2), round(x2[0], 2), round(y2[0], 2)]

    return xyxy_

# def apply_homography_xyxy(xyxy, H):
#     xyxy_ = np.zeros_like(xyxy)  # 确保数组类型是浮点数类型，以防止精度问题
#
#     for idx in range(xyxy.shape[0]):
#         x1, y1, x2, y2 = xyxy[idx]
#
#         # Apply homography transformation
#         pt1_homogeneous = H @ np.array([x1, y1, 1]).reshape(3, 1)
#         pt2_homogeneous = H @ np.array([x2, y2, 1]).reshape(3, 1)
#
#         x1_transformed = pt1_homogeneous[0, 0] / pt1_homogeneous[2, 0]
#         y1_transformed = pt1_homogeneous[1, 0] / pt1_homogeneous[2, 0]
#
#         x2_transformed = pt2_homogeneous[0, 0] / pt2_homogeneous[2, 0]
#         y2_transformed = pt2_homogeneous[1, 0] / pt2_homogeneous[2, 0]
#
#         xyxy_[idx] = [x1_transformed, y1_transformed, x2_transformed, y2_transformed]
#
#     return xyxy_
...
def draw_tracks(image, tracks, ids_dict, src, classes=None):
    ...
    if len(tracks) == 0:
        return vis  # If no tracks, return the original image
